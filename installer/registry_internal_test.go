...
var _ = Describe("Byohost Installer Tests", func() {
	...
	Context("When supported registry is fetched", func() {
		...
		It("Should match with the supported os and k8s versions", func() {
			...
			Expect(osFilters).To(ContainElements("Ubuntu_22.04.*_LTS_x86-64"))
			Expect(osFilters).To(HaveLen(2))
			Expect(osBundles).To(ContainElements("Ubuntu_20.04.1_x86-64"))
			Expect(osBundles).To(ContainElements("Ubuntu_22.04.1_LTS_x86-64"))
			Expect(osBundles).To(HaveLen(2))

			osBundleResult20_04 := r.ListK8s("Ubuntu_20.04.1_x86-64")
			Expect(osBundleResult20_04).To(ContainElements("v1.24.*", "v1.25.*", "v1.26.*"))
			Expect(osBundleResult20_04).To(HaveLen(3))

			osBundleResult22_04 := r.ListK8s("Ubuntu_22.04.1_LTS_x86-64")
			Expect(osBundleResult22_04).To(ContainElements("v1.24.*", "v1.25.*", "v1.26.*"))
			Expect(osBundleResult22_04).To(HaveLen(3))
		})
	})
})
